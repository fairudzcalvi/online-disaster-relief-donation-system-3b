...
// Update inventory summary stats
function updateInventorySummary() {
  const total = items.reduce((s, i) => s + (parseInt(i.quantity) || 0), 0);
  const pending = items.filter(i => i.status === 'pending').reduce((s, i) => s + (parseInt(i.quantity) || 0), 0);
  const allocated = items.filter(i => i.status === 'allocated').reduce((s, i) => s + (parseInt(i.quantity) || 0), 0);
  const distributed = items.filter(i => i.status === 'distributed').reduce((s, i) => s + (parseInt(i.quantity) || 0), 0);

  const el = (id, val) => { const e = document.getElementById(id); if (e) e.textContent = val; };
  el('totalReceived', total);
  el('totalAvailable', pending);
  el('totalAllocated', allocated);
  el('totalDistributed', distributed);
}
...
async function saveItem() {
  ...
  formData.append("donorName", document.getElementById('donorName').value.trim());
  formData.append("dateReceived", document.getElementById('dateReceived').value);
  formData.append("storageLocation", document.getElementById('storageLocation').value.trim());
  formData.append("expiryDate", document.getElementById('expiryDate').value);
  formData.append("notes", document.getElementById('notes').value.trim());

  try {
    const response = await fetch("../api/auth/save_item.php", {
      method: "POST",
      headers: { 'Authorization': 'Bearer ' + (sessionStorage.getItem('adminToken') || '') },
      body: formData
    });
    ...
    if (result.status === "success") {
      ...
      showNotification(result.message || "Error saving item", "error");
    }
  } catch (e) {
